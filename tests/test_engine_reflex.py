...
async def test_reflex_fast_path_success(
    ...
    tool_name = intent.get("tool_name") if isinstance(intent, dict) else getattr(intent, "tool_name", None)
    assert tool_name == "fast_tool"
...
async def test_reflex_fast_path_fallback_invalid_intent(
    ...
    message = intent.get("message") if isinstance(intent, dict) else getattr(intent, "message", None)
    assert message == "Deep thought result"
